...
CREATE POLICY "Users can delete their own profile"
  ON public.teachers FOR DELETE USING (auth.uid() = id);

-- Auto-insert into teachers when a new auth.users row is created
...
-- Create the students table (group_id FK added after groups exists)
...
CREATE POLICY "Teachers can view their own students"
  ON public.students FOR SELECT USING (auth.uid() = teacher_id);
...
CREATE POLICY "Teachers can delete their own students"
  ON public.students FOR DELETE USING (auth.uid() = teacher_id);
...
-- Add group_id FK to students now that groups exists
...
CREATE POLICY "Teachers can delete their own sessions"
  ON public.sessions FOR DELETE USING (auth.uid() = teacher_id);

-- Wire sessions FK onto groups
...
CREATE POLICY "Teachers can view their own students progress"
  ON public.student_progress FOR SELECT USING (auth.uid() = teacher_id);
...
CREATE POLICY "Teachers can delete their own students progress"
  ON public.student_progress FOR DELETE USING (auth.uid() = teacher_id);
...
CREATE POLICY "Teachers can view their own whiteboard states"
  ON public.whiteboard_states FOR SELECT USING (auth.uid() = teacher_id);

CREATE POLICY "Teachers can insert their own whiteboard states"
  ON public.whiteboard_states FOR INSERT
  WITH CHECK (
    auth.uid() = teacher_id
    AND EXISTS (SELECT 1 FROM public.teachers WHERE id = auth.uid())
  );
...
CREATE POLICY "Teachers can update their own whiteboard states"
  ON public.whiteboard_states FOR UPDATE
  USING (auth.uid() = teacher_id)
  WITH CHECK (EXISTS (SELECT 1 FROM public.teachers WHERE id = auth.uid()));

CREATE POLICY "Teachers can delete their own whiteboard states"
  ON public.whiteboard_states FOR DELETE USING (auth.uid() = teacher_id);
